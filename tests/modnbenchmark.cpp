...
int main() {
  ...
  auto fmtime =
      time([M, mod](uint64_t v) { return fastmod_u64(v, M, mod); }, zomg);
  ...
  auto cfmtime =
      time([constantM, constantmod](
               uint64_t v) { return fastmod_u64(v, constantM, constantmod); },
           zomg);
  std::cout << "timing x modulo constantmod; " << std::endl;
  auto cmodtime =
      time([constantmod](uint64_t x) { return x % constantmod; }, zomg);
  uint64_t t = mod - 1;
  t |= t >> 1;
  t |= t >> 2;
  t |= t >> 4;
